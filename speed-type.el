...
(defun speed-type--handle-del (start end)
  ...
  (let* ((pos (+ (1- start)))
	 ...
	 (diff (- end start)))
    (if (= diff 0)
	(progn (unless (= 0 q) (cl-incf speed-type--remaining)))
      (dotimes (i diff)
	(let* ((pos (+ (1- start) i))
	       (q (aref speed-type--mod-str pos)))
	  (cond ((= q 0) ())
...
END is a point where the check stops to scan for diff."
  (let ((start0 (1- start))
	...
	(when (and (not (string-blank-p (char-to-string (aref orig i))))
		   (or (eq speed-type-point-motion-on-error 'point-move) correct))
