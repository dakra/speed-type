...
Random samples are taken from Project Gutenberg. A small number of
books will be downloaded on demand and stored in
"~/emacs.d/speed-type". They will only be downloaded once.

You can configure a language (var `speed-type-default-lang`) which
will pick books of your language. Beaware that gutenberg does not have
hundreds of books for all languages.
...
## Documentation
A little diagram because [uninline](https://github.com/tbanel/uniline) is fun:
```uniline
                          ╭────╮   ╭─────╮         ╭────────────╮
...
After setup is complete it kills the completed speed-type- and
preview-buffer. It may reuse the existing content-buffer.
