   ...
   (t
    (dotimes (pos (length str))
      (let ((this-command 'self-insert-command)
            ...
            (expand-abbrev nil)
            (internal-auto-fill nil)
            (auto-fill-function nil)
            (auto-fill-chars nil))
        (self-insert-command 1 (aref str pos)))))))
  ...
  (let ((b-speed-type-point-motion-on-error speed-type-point-motion-on-error))
    (unwind-protect
        (progn
          ...
          (speed-type-test-region
           (lambda ()
             ;; TODO Enable overwrite-mode here, make point-movement work in test-env
             ;; For some reason it resets the point due to some post-hook or post-advice.
             ;; (when (= (% (random 2) 2) 0) (overwrite-mode))
             (should (= (point) 1))
             (should (= speed-type--errors 0))
             (should (= speed-type--non-consecutive-errors 0))
             (speed-type-test-user-input "b")
             ...
             (should (= speed-type--non-consecutive-errors 1))
             (should (= (point) 2))
             (speed-type-test-user-input "DEL")
             (should (= (point) 1))
             (speed-type-test-user-input "a")
             (should (= speed-type--errors 2))
  ...
  (let ((content "abcde\n\nab\tcde")
    ...
    (with-temp-buffer
      ...
      (let ((buf (speed-type-region (point-min) (point-max)))
        ...
        (unwind-protect
            (with-current-buffer buf
              (when override-active (overwrite-mode))
              (speed-type-test-user-input "a")
              (speed-type-test-user-input "b")
              (speed-type-test-user-input "a")
              (speed-type-test-user-input "a")
              ...
              (speed-type-test-user-input "a")
              (speed-type-test-user-input "b")
              (speed-type-test-user-input "TAB")
              ...
              (dotimes (i 3)
                (should (eq (overlay-get (car (overlays-at (+ 6 i))) 'face) 'speed-type-correct-face)))
              )
          (kill-buffer buf)
...
(defvar file-location nil)
(eval-and-compile
  (setq file-location (file-name-directory (or load-file-name buffer-file-name (temporary-file-directory)))))
(ert-deftest speed-type-test/setup-replay ()
  "Test that replay contains the exact same content again."
  (let ((speed-type-default-lang "de")
        (speed-type-stop-words nil)
        (speed-type-directory file-location)
        (speed-type-save-statistic-option 'never)
        (speed-type-randomize t)
        (speed-type-add-extra-words-on-error 0)
        (speed-type-add-extra-words-on-non-consecutive-errors 0))
    (dolist (cmd
             (list (list 'speed-type-text)
                   (list 'speed-type-text-top-x 50)
                   (list 'speed-type-top-x 50)
                   (list 'speed-type-top-100)
                   (list 'speed-type-top-1000)
                   ;; TODO Requires pandoc to be installed in testing-sandbox
                   ;; (list 'speed-type-pandoc "https://en.wikipedia.org/wiki/Haskell")
                   ;; (list 'speed-type-pandoc-top-x "https://en.wikipedia.org/wiki/Haskell" 50)
                   ))
      (message "Running %s..." (symbol-name (car cmd)))
      (with-current-buffer (if (cdr cmd) (apply (car cmd) (cdr cmd)) (funcall (car cmd)))
        (let ((before-content (replace-regexp-in-string "[[:space:]\n]" "" (buffer-substring-no-properties (point-min) (point-max)))))
          (insert "a")
          (with-current-buffer (speed-type-replay)
            (should (not (string-blank-p before-content)))
            (should (string= (replace-regexp-in-string "[[:space:]\n]" "" (buffer-substring-no-properties (point-min) (point-max))) before-content)))
          (speed-type-quit)))
      (message "Running %s...DONE" (symbol-name (car cmd))))))
